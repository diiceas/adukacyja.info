...
(function (ctx, $) {
    ...
    ctx.extend(htmlClass, ctx.fieldClass, {

        startNode: null, endNode: null, spaceHolder: null,

        initialize: function () {
            if (!this.endNode) {
                var find = ctx.widgetMask.prototype.findEndNode;
        ...
        ensureEditSpace: function () {
            ...
            if (empty) {
                var fn = ctx.widgetMask.prototype.genStyle;
                var pos = fn.call({
                    startNode: this.startNode,
                    endNode: this.endNode
                });
        ...
        wrap: function () {
            ...
            while (wrapperDom.nextSibling != end)
                wrapperDom.appendChild(wrapperDom.nextSibling);
            return wrapper;
        }
    });

    // register
    ctx.htmlClass = htmlClass;
